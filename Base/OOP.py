...
# -----   Класс, который может работать внутри менеджера контекста with   -------------------------------------------------------------------------------------------------------------


class MyClass:
    # метод вызываеться при входе в контекст
    def __enter__(self):
        self.connection = 1

    # метод вызываеться при выходе из контекста , или когда вызываеться исключение Exception
